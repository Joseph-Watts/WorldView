# WorldView Online - Step 10
# Add a standalone browser validation page that checks the static app's data
# and calculations against the R reference fixtures created in Step 5.

app_dir <- file.path(getwd(), "worldview_static_app")
validation_html_path <- file.path(app_dir, "validation.html")
...
<head>
  ...
  <title>WorldView validation</title>
  ...
  <header class="site-header">
    <div class="header-inner">
      <a class="brand" href="index.html#home">
        ...
        <span><strong>WorldView</strong><small>Browser validation</small></span>
      </a>
      <a class="button button-secondary" href="index.html#home">Return to app</a>
    </div>
  </header>
  ...
  <main id="main-content" tabindex="-1">
    <section class="page">
      <div class="page-heading">
        ...
        <p>This page recalculates selected summaries and correlations in the browser and compares them with the R reference fixtures.</p>
      </div>

      <div id="validation-status" class="notice" aria-live="polite">Running validation checks...</div>

      <div class="stat-cards" aria-label="Validation totals">
        <div class="stat-card"><strong id="validation-total">0</strong><span>Total checks</span></div>
        <div class="stat-card"><strong id="validation-passed">0</strong><span>Passed</span></div>
        <div class="stat-card"><strong id="validation-failed">0</strong><span>Failed</span></div>
      </div>

      <div class="table-wrap">
        <table class="results-table validation-table">
          <thead><tr><th>Check</th><th>Expected</th><th>Browser result</th><th>Status</th></tr></thead>
      ...
      <details class="full-table-disclosure">
        ...
        <div class="disclosure-content">
          <ul>
            <li>Counts and sample sizes must match exactly.</li>
            <li>Means, standard deviations, quartiles, and correlations use a tolerance of 1e-10.</li>
            <li>Confidence limits use a tolerance of 1e-8.</li>
...
writeLines(validation_html, validation_html_path, useBytes = TRUE)
writeLines(validation_js, validation_js_path, useBytes = TRUE)

css <- paste(readLines(css_path, warn = FALSE), collapse = "\n")
if (!grepl("Step 10: browser validation", css, fixed = TRUE)) cat(validation_css, file = css_path, append = TRUE)

validation <- data.frame(
  check = c(
    "validation_html_created",
    "validation_javascript_created",
    "reference_fixture_linked",
    "summary_fixture_checks_added",
    "correlation_fixture_checks_added",
    "manifest_checks_added",
    "validation_styles_added"
  ),
  passed = c(
    file.exists(validation_html_path),
    file.exists(validation_js_path),
    grepl("worldview-reference-fixtures-v1.0.0.json", validation_js, fixed = TRUE),
    grepl("testSummaryFixtures", validation_js, fixed = TRUE),
    grepl("testCorrelationFixtures", validation_js, fixed = TRUE),
    grepl("Manifest participant count", validation_js, fixed = TRUE),
    grepl("Step 10: browser validation", paste(readLines(css_path, warn = FALSE), collapse = "\n"), fixed = TRUE)
  ),
  stringsAsFactors = FALSE
)

write.csv(validation, file.path(app_dir, "step10_validation_checks.csv"), row.names = FALSE, na = "")
if (!all(validation$passed)) stop("Step 10 build validation failed.")

cat("\nStep 10 completed successfully.\n\n")
cat("Restart the server, then open:\n")
cat("  http://127.0.0.1:<port>/validation.html\n\n")
cat("Use the port printed by servr::httd().\n")
