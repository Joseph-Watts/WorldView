...
d_vars_coded <- readxl::read_xlsx("WVS_Dataset/Codebook manual coded index.xlsx")
#d_vars_coded <- readxl::read_xlsx("WVS_Dataset/WVS7_Codebook_updated_labels.xlsx")
...
#' subset by country
country_sum <- function(data, country, cols = cols_to_sum_fun) {
  #' Country wanted
  i_d <- data[data$B_COUNTRY == country, ]
  
  #' Applying sum_fun to columns
  i_d_sum <- lapply(cols_to_sum_fun, sum_fun, data = i_d)
