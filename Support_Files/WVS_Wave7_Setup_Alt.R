...
wvs7_var_index <- readxl::read_xlsx("WVS_Dataset/WVS7_Variable_Index.xlsx")
...
for(i in 1:length(cols_display)){
  ...
  if(i_display_type %in% 
    ...
    i_factor_ordered <- wvs7_var_index$Variable_Display_Type[i] == "factor_ordered"
...
factor_cols_display <- wvs7_var_index_displayed$Col_ID[
  wvs7_var_index_displayed$Variable_Display_Type %in% 
    c("factor_ordered", "factor")
  ]
...
for (section_i in unique(wvs7_var_index_displayed$Section)) {
  ...
  for (i in seq_len(nrow(section_df))) {
    ...
    md_lines <- c(
      md_lines,
      paste0("**Variable class:** ", md_clean(row_i$Processed_Data_Type)),
      ""
    )
